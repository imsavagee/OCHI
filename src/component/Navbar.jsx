...
function Navbar() {
    ...
    return (
        <div style={nav}
            className='fixed top-0 z-[999] w-full px-[3vw] py-[1vw] font-["Neue Montreal"] flex justify-between items-center'>
            <div className='logo'>
            ...
            <div className='links flex gap-[3vw] max-[650px]:hidden '>
                {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map((item, index) => (
                    <a className={`text-lg font-light ${index === 4 && "ml-[9.5vw]"}`} key={index}  >{item}</a>
                ))}
            </div>
        </div>
    )
